...
SOURCE = ROOT / "src" / "Paralogic" / "EvidenceOverclaim.lean"
OUT_DIR = ROOT / "docs" / "evidence_overclaim_checks"
...
REQUIRED_POSITIVE_ARTIFACTS: tuple[str, ...] = (
    ...
    "EvidenceOverclaimProfile_to_supportDegraded",
    "EvidenceOverclaimProfile.toMechanismProfile",
    "EvidenceOverclaimProfile_to_mechanism_satisfied",
    "M1EvidenceOverclaimCase",
    "M1EvidenceOverclaimSatisfied",
    "M1EvidenceOverclaimCase_to_ISFSem",
...
REQUIRED_BLOCKERS: tuple[str, ...] = (
    "missing_claim_scope_blocks_overclaim",
    "irrelevant_evidence_blocks_overclaim",
    "sufficient_evidence_blocks_overclaim",
    "matched_scope_blocks_overclaim",
...
def condition_coverage(text: str | None = None) -> dict[str, dict[str, bool]]:
    text = source_text() if text is None else text
    profile_block = extract_structure_block(text, "EvidenceOverclaimProfile")
    satisfied_block = extract_structure_block(
        text, "EvidenceOverclaimProfileSatisfied"
    )
    blocked_block = re.search(
        r"def\s+EvidenceOverclaimProfileBlocked[\s\S]*?"
        r"(?=\ntheorem\s+missing_claim_scope_blocks_overclaim)",
        text,
    )
    if blocked_block is None:
        raise ValueError("could not locate EvidenceOverclaimProfileBlocked")
