...
SOURCE = ROOT / "src" / "Paralogic" / "MetricProxy.lean"
OUT_DIR = ROOT / "docs" / "metric_proxy_checks"
...
REQUIRED_POSITIVE_ARTIFACTS: tuple[str, ...] = (
    ...
    "MetricProxyProfile_to_supportDegraded",
    "MetricProxyProfile.toMechanismProfile",
    "MetricProxyProfile_to_mechanism_satisfied",
    "M2MetricProxyCollapseCase",
    "M2MetricProxyCollapseSatisfied",
    "M2MetricProxyCollapseCase_to_ISFSem",
...
REQUIRED_BLOCKERS: tuple[str, ...] = (
    "missing_metric_target_blocks_profile",
    "unused_metric_proxy_blocks_profile",
    "absent_metric_pressure_blocks_profile",
    "aligned_metric_proxy_blocks_profile",
    "immaterial_metric_replacement_blocks_profile",
    "guarded_metric_boundary_blocks_profile",
...
def condition_coverage(text: str | None = None) -> dict[str, dict[str, bool]]:
    text = source_text() if text is None else text
    profile_block = extract_structure_block(text, "MetricProxyProfile")
    satisfied_block = extract_structure_block(text, "MetricProxyProfileSatisfied")
    blocked_block = re.search(
        r"def\s+MetricProxyProfileBlocked[\s\S]*?"
        r"(?=\ntheorem\s+missing_metric_target_blocks_profile)",
        text,
    )
    if blocked_block is None:
        raise ValueError("could not locate MetricProxyProfileBlocked")
