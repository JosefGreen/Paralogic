...
36a.9. M9 now has a mechanism-specific operational profile in
    `VetoSuppression.lean`: veto right, affected participant, veto attempt,
    suppression, materiality, absent review path, and missing boundary guard
    jointly project to support degradation and ISF, while each missing
...
36a.11. M11 now has a mechanism-specific operational profile in
    `SymbolicOverload.lean`: declared symbol, multiple interpretations,
    interpretive conflict, material audience uptake, claim-dependence on the
    overloaded symbol, missing disambiguation, missing evaluator resolution,
    and missing boundary guard jointly project to support degradation and ISF,
    while each missing condition has a concrete blocked witness.
    Source-complete and empirical M11 validation remain open.
...
37b. Full empirical-validation warrant discharge now has a protocol-level
    operational model requiring validation, construct-validity, reliability,
    and replication predicates, with negative controls for nominal validation,
...
37d. Operational warrant coverage is now itself a Lean theorem:
    `OperationalWarrantCoverageComplete` proves every currently enumerated
    `WarrantObligation` is operationally discharged in the scoped core, while
    generic guards keep that status distinct from source-backed or empirical
    promotion.  Future warrant additions must extend this theorem.
